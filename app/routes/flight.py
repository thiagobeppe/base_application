...
def update_flight(
    ...
    db = Depends(deps.get_db),
    flight_id: int,
    flight_value: FlightUpdate
    ):
    actual_reccord = db.query(flight_model).filter(flight_model.id == flight_id).one()
    
    actual_reccord.id = actual_reccord.id,
    actual_reccord.ts_departure = actual_reccord.ts_departure if flight_value.ts_departure == None else _transform_datetime(flight_value.ts_departure),
    actual_reccord.ts_arrival = actual_reccord.ts_arrival if flight_value.ts_arrival == None else _transform_datetime(flight_value.ts_arrival),
